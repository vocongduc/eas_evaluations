...
@section('content')
    <div class="container-fluid">
        ...
        <input type="hidden" value="{{$form_id}}" class="form-id">
        <input type="hidden" value="{{auth()->user()->id}}" class="evaluate-user-id">
            ...
            <div class="col-12">
                <div class="row justify-content-center">
                    <form action="{{url('evaluations/evaluate').'/'.$user['id'].'/'.$form_id}}"
                        ...
                        top: 2rem;">
                            <a href="">
                                <button type="submit" class="btn btn-primary mr-4 " id="submitData">Hoàn tất</button>
                            </a>
                            <a href="{{route('evaluations.index')}}">
                                <button type="button" class="btn btn-secondary ">Trở lại</button>
                            </a>
                        </div>
                        <input type="hidden" value="" class="mainPointId" name="mainPointId">
                        <input type="hidden" value="" class="totalPoint" name="totalPoint">
                            ...
                            @foreach($mainPoints as $mainPointId => $mainPoint)
                                ...
                                @foreach($mainPoint['categories'] as $category)
                                    ...
                                    @foreach($category['criterias'] as $criteria)
                                        <tr>
                                            ...
                                            <td><input type="number" required min="1" max="{{$criteria->point_max}}"
                                                       ...
                                                       value="{{auth()->user()->hasRole('member') ? "" : $point['point']}}"
                                                       @endif
                                                       @endforeach
                                                       onkeyup="memberPoint()" onchange="member(); sumPointUser(); getPointMember()"/></td>
                                            <td><input type="text"
                                            ...
                                            <td><input type="text"
                                                       ...
                                                       name="" value="{{!empty($criteria->team_point)
                                                            ?substr($criteria->team_point,0,3) : ''}}"
                                                       disabled/></td>
                                            <td><input type="text"
                                            ...
                                            <td><input type="number" required min="1" max="{{$criteria->point_max}}"
                                                       ...
                                                       @endif
                                                       @endforeach
                                                       onkeyup="mentorPoint()" onchange="mentor(); sumPointUser(); getPointMentor()"/></td>
                                            <td><input type="text"
                                            ...
                                            <td><input type="number" required min="1" max="{{$criteria->point_max}}"
                                                       ...
                                                       onkeyup="myFunction()" onchange="admin(); sumPointUser(); getPointAdmin()"/></td>
                                            <td><input type="text"
                                                       class="form-control total-point total-point-{{$mainPointId}}"
                                                       value="" disabled/>
                                            </td>
                                            @endhasrole
                                        </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <th></th>
                                        <th class="bg-info text-white total-point-criteria-member-{{$mainPointId}} total-criteria-point-member total-criteria-point-member-{{$mainPointId}} total-cri-point-{{$mainPointId}}">
                                            0
                                        </th>
                                        @hasanyrole('admin|mentor')
                                        <!-- team  point  -->
                                        <td></td>
                                        <th class="bg-info text-white total-point-criteria-{{$mainPointId}}  total-criteria-point-team-{{$mainPointId}} total-cri-point-{{$mainPointId}}">
                                            0
                                        </th>
                                        <!-- mentor point  -->
                                        <td></td>
                                        <th class="bg-info text-white total-criteria-point total-point-criteria-mentor-{{$mainPointId}} total-criteria-point-mentor-{{$mainPointId}} total-cri-point-{{$mainPointId}}">
                                            0
                                        </th>
                                        @endhasanyrole
                                        <!-- admin point  -->
                                        @hasrole('admin')
                                        <td></td>
                                        <th class="bg-info text-white total-criteria-point-admin total-point-criteria-admin-{{$mainPointId}} total-criteria-point-{{$mainPointId}} total-cri-point-{{$mainPointId}}">
                                            0
                                        </th>
                                        @endhasrole
                                    </tr>
                        ...
                        <table class="table table-bordered table-dark">
                            ...
                            <tr style="height:100px; font-size:2rem;vertical-align: middle; text-align: center;line-height: 100px;">
                                ...
                                <th scope="col" class="total-point-user">0</th>
